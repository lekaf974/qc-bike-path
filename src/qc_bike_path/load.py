...
class BikePathDataLoader:
    ...
    async def save_record(self, record: BikePathRecord) -> bool:
        ...
        try:
            ...
            filter_criteria = (
                {"id": record.id} if record.id else {"_id": record_dict.get("_id")}
            )
    ...
    async def save_records_batch(self, records: list[BikePathRecord]) -> dict[str, int]:
        ...
        try:
            ...
            logger.warning(
                ...
                updated=stats["updated"],
                errors=stats["errors"],
                error_details=e.details.get("writeErrors", [])[
                    :5
                ],  # Log first 5 errors
            )
    ...
    async def save_geojson(self, geojson_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            geojson_document = {
                **geojson_data,
                "stored_at": geojson_data.get("metadata", {}).get(
    ...
    async def get_collection_stats(self) -> dict[str, Any]:
        ...
        try:
            stats = await self.database.command(
                "collStats", settings.mongodb_collection
            )
            ...
            return {
                ...
                "latest_extraction": (
                    latest_record.get("extraction_timestamp") if latest_record else None
                ),
            }
