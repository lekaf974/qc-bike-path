...
class BikePathDataExtractor:
    ...
    async def fetch_bike_path_data(self, limit: int | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if "result" not in data:
                raise DataExtractionError(
                    "Invalid response format: missing 'result' key"
                )

            result = data["result"]
            if "records" not in result:
                raise DataExtractionError(
                    "Invalid response format: missing 'records' in result"
                )

            records = result["records"]
            logger.info(
                "Successfully extracted bike path data", record_count=len(records)
            )
    ...
    async def fetch_geojson_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "GeoJSON format not available, falling back to regular data format"
            )
...
async def extract_bike_path_data(limit: int | None = None) -> dict[str, Any]:
