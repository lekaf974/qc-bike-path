...
class BikePathETLPipeline:
    ...
    async def run_extract_phase(self, limit: Optional[int] = None) -> dict:
    ...
    async def run_load_phase(self, transformed_records: list, geojson_data: dict) -> dict:
    ...
    async def run_full_pipeline(self, limit: Optional[int] = None) -> dict:
...
async def main() -> None:
    ...
    try:
        stats = await pipeline.run_full_pipeline(limit=limit)
        ...
        print(f"🗺️  GeoJSON saved: {'✅' if stats['geojson_saved'] else '❌'}")
