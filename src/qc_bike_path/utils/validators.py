...
def validate_mongodb_connection_string(connection_string: str) -> bool:
    ...
    return connection_string.startswith("mongodb://") or connection_string.startswith(
        "mongodb+srv://"
    )
...
def sanitize_string_field(value: Any, max_length: int | None = None) -> str | None:
    ...
    if max_length and len(value) > max_length:
        logger.warning(
            "String field truncated", original_length=len(value), max_length=max_length
        )
...
def validate_numeric_range(
    value: int | float, min_val: float | None = None, max_val: float | None = None
) -> bool:
...
def validate_bike_path_record(record: dict[str, Any]) -> list[str]:
    ...
    if (
        "geometry" in record
        and record["geometry"]
        and not validate_geojson_geometry(record["geometry"])
    ):
    ...
    if "length_km" in record and record["length_km"] is not None:
        try:
            ...
            if not validate_numeric_range(
                length, min_val=0, max_val=1000
            ):  # 1000km seems reasonable max
    ...
    if (
        "name" in record
        and record["name"]
        and len(str(record["name"])) > 500  # Reasonable limit for path names
    ):
